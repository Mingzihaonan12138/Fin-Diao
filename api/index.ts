...
// file with only bare (npm) imports so Vercel's bundler ships it cleanly —
...
// Azure 神经语音代理：密钥只在服务端，前端拿不到。区域默认 northeurope（用户的 suomi-tts 资源）。
  ...
  try {
    ...
    const keysList = snapshot.docs.map((doc) => {
      ...
      const maskedKey = rawKey.length > 10
        ...
        : "****";
      return { id: doc.id, maskedKey, addedAt: data.addedAt };
    });
    res.json(keysList);
  } catch (error: any) {
    ...
    for (let i = 0; i < keysPool.length; i++) {
      ...
      try {
        ...
        if (fileData && fileType) {
          contents.push({ inlineData: { mimeType: fileType, data: fileData } });
        }
